...
#define FAIL(v) { ERROR("%s:%d FAIL %s",basename((char*)__FILE__),__LINE__,v); local_failures++; }

#define FAIL_IF(v) if (v){ ERROR("%s:%d FAIL IF %s",basename((char*)__FILE__),__LINE__,#v); local_failures++; } else { local_successes++; }
#define FAIL_IF_NOT(v) if ( !(v) ){ ERROR("%s:%d FAIL IF NOT %s",basename((char*)__FILE__),__LINE__,#v); local_failures++; } else { local_successes++; }


#define FAIL_IF_EQUAL(A,B) if ((A)==(B)){ ERROR("%s:%d FAIL IF EQUAL %s == %s",basename((char*)__FILE__),__LINE__,#A,#B); local_failures++; } else { local_successes++; }
#define FAIL_IF_NOT_EQUAL(A,B) if ((A)!=(B)){ ERROR("%s:%d FAIL IF NOT EQUAL %s != %s",basename((char*)__FILE__),__LINE__,#A,#B); local_failures++; } else { local_successes++; }

#define FAIL_IF_EQUAL_INT(A,B) if ((A)==(B)){ ERROR("%s:%d FAIL IF EQUAL %s == %s, %d == %d",basename((char*)__FILE__),__LINE__,#A,#B,A,B); local_failures++; } else { local_successes++; }
#define FAIL_IF_NOT_EQUAL_INT(A,B) if ((A)!=(B)){ ERROR("%s:%d FAIL IF NOT EQUAL %s != %s, %d == %d",basename((char*)__FILE__),__LINE__,#A,#B,A,B); local_failures++; } else { local_successes++; }
